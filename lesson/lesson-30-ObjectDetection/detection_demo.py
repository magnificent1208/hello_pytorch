...
如何实现目标检测
    将3D张量映射到两个张量
        a.分类张量：shape [N,NumClass+1]
        b.边界框张量: shape [N,4]
    边界框N如何确定？
        传统滑框策略 sliding window
        利用卷积减少重复计算(因为特征图里面一个像素点，已经可以代表原图里面的一块区域)
'''
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        output_list = model(input_list) #放进模型的，不再是一个由许多3d拼成的4D tensor 而是一组list中的3D tensor
