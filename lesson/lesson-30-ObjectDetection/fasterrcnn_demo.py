...
3. regressor [num_anchors*4, h_f, w_f]
4. NMS out [n_proposals=2000, 4]
5. ROI layer [512, 256, 7, 7]
6. FC1 FC2 [512, 1024]
7. class+1 softmax [512, c+1]
8. regressor [512, (c+1)*4]
'''
...
class GeneralizedRCNN(nn.Module)
    forward()
        1. features = self.backbone(image.tensors)
        2. proposals, proposal_losses = self.rpn(images, features, targets) #包含了NMS
        3. detections, detector_losses = self.roi_heads(features, proposals, images.images_size, targets)

class FasterRCNN(GeneralizedRCNN)
torchvision.models.detection.fasterrcnn_resnet50_fpn() 返回FasterRCNN实例
'''
...
if __name__ == "__main__":
    ...
    train_transform = Compose([ToTensor(), RandomHorizontalFlip(0.5)]) #这里图像预处理，对图像和标签同时执行。
