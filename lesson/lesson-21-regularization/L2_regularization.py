...
(泛化)误差= 偏差+方差+噪声
        误差error： 由一下几个组件构成的差异
        偏差bias：度量了学习算法的期望预测和真实结果的偏离程，即刻画了学习本身的你和能力。
        方差variance：度量了同样大小的训练集的变动，刻画数据扰动造成的影响 *过拟合
        噪声noise：样本标注与真实标注之间的偏离 *数据标不对

而机器学习里面的正则化，就是对于方差的处理。

L1 绝对值求和 *希望权值的绝对值足够小
L2 平方求和 *希望权值的平方足够小
----------------------------------------
L2正则项 又称为weight decay 权值衰减

Obj function = Cost + Regularization Term
在训练当中：
Obj = Loss + lamda*0.5*sum(w[i]**2) #因为求导的时候想把平方去掉 所以加了个1/2
w的更新：
    #定义loss
    def loss(x)：
        return y
    #推导加入正则项之后的权值更新公式
    w[i+1] = w[i] - ( diff(loss(w[i]),w[i]) + lamda*w[i] )
           = (1-lamda)*w[i] - diff(loss(w[i]),w[i])
    # 由上看出 lambda如何控制权值衰减 （ 0<lambda<1 )
'''
