...
if __name__ == '__main__':
    ...
    for epoch in range(num_epochs):
        for i, data in enumerate(train_loader):
            ...
            real_label = torch.full((b_size,), real_idx, device=device, dtype=torch.float32) #在pytorch中float32为float类型，而float64则为double类型 记得对齐

            noise = torch.randn(b_size, nz, 1, 1, device=device)
            fake_img = net_g(noise)
            fake_label = torch.full((b_size,), fake_idx, device=device, dtype=torch.float32)
